...
### Manual SMB installation

Use this only when replacing an existing test installation. Stop the game first, then copy the launcher and `nfsmw/` directory to the SD card share:

```sh
# Create a temporary SMB auth file; do not put the password in the command line.
printf 'username = spruce\npassword = %s\n' "$(tr -d '\r\n' < ~/tsps.txt)" > /tmp/tsps-smb.auth
chmod 600 /tmp/tsps-smb.auth

smbclient //192.168.50.135/SDCARD -A /tmp/tsps-smb.auth -m SMB3 \
  -c 'put "portmaster/Need for Speed Most Wanted.sh" "Roms/PORTS/Need for Speed - Most Wanted.sh"'

smbclient //192.168.50.135/SDCARD -A /tmp/tsps-smb.auth -m SMB3 \
  -c 'cd Roms/PORTS/nfsmw; put runtime/build/nfsmw_mapper nfsmw_runtime'

rm -f /tmp/tsps-smb.auth
```

Verify the deployed runtime hash before launching. Always back up the current runtime and launcher before an experimental replacement.
